<?php


class ProdutoModel extends CI_Model {
    function __construct() {

        parent::__construct();

        $this->load->model('admin/AcessoLoginModel');

        $this->tabela = 'PRODUTO';

    }

    function dados($dados = array()) {

        $array = array();

        $array['id'] =null;

        $array['nome'] =null;

        $array['preco_venda'] =null;

        $array['preco_custo'] =null;

        $array['estoque'] =null;

        $array['data'] =null;

        $array['horas'] =null;

        $array['admin'] =$this->AcessoLoginModel->verAdmin();

        $array['exclusao'] =null;

        foreach ($array as $key => $value) {

             if(isset($dados[$key])){

                 $array[$key]=$dados[$key];

             } else{

               if(is_null($array[$key])){

                   $array[$key] = null;

                }
              }

        }


        return $array;
    }



    function incluir($dados) {

        $dados=array_filter($this->dados($dados));

        $dados['data'] =date("Y-m-d");

        $this->db->insert("PRODUTO", $dados);

        return $this->db->insert_id();
    }

    function editar($dados) {

        $dados = array_filter($this->dados($dados));

        $this->db->where('PRODUTO.id', $dados['id']);

        $this->db->where('PRODUTO.exclusao is null');

        $this->banco->usuario("PRODUTO");

        $this->db->update("PRODUTO", $dados);
    }

    function listar($maximo = NULL, $inicio = NULL) {

        $this->db->from("PRODUTO");

        $this->db->where('PRODUTO.exclusao is null');

        $this->banco->usuario("PRODUTO");

        $query = $this->db->get("", $maximo, $inicio);

        return $query->result_array();
    }

    function filtrar($filtro=array(),$maximo = NULL, $inicio = NULL){

      $ordem= "";

      if(!empty($filtro['ordem'])){

        $ordem= $filtro['ordem'];

      }
      unset($filtro['ordem']);

        $filtro=array_filter($this->dados($filtro));

        $this->db->from("PRODUTO");

        if(!empty($ordem)){

            $this->db->order_by('PRODUTO.nome',$ordem);

        }

        $this->db->like($filtro);

        $this->db->where('PRODUTO.exclusao is null');

        $this->banco->usuario("PRODUTO");

        $query = $this->db->get("", $maximo, $inicio);

        return $query->result_array();

    }

    function ver($id) {

        $this->db->from("PRODUTO");

        $this->db->where('PRODUTO.id', $id);

        $this->db->where('PRODUTO.exclusao is null');

        $this->banco->usuario("PRODUTO");

        $query = $this->db->get();

        return $query->row_array();
    }

    function excluir($id) {

        $dados = array();

        $this->db->where('PRODUTO.id', $id);

        $this->banco->usuario("PRODUTO");

        $dados['exclusao']=date("Y-m-d");

        $this->db->update("PRODUTO",$dados);

    }

    function deletar($id) {

        $this->db->where('PRODUTO.id', $id);

        $this->db->where('PRODUTO.exclusao not null');

        $this->banco->usuario("PRODUTO");

        $this->db->delete("PRODUTO");

    }

    function contarTotal($filtro=false) {

        $this->db->select('*');

        $this->db->select('PRODUTO.id as id');

        $this->db->from("PRODUTO");

        if(!empty($filtro['ordem'])){

          $ordem= $filtro['ordem'];

        }
        unset($filtro['ordem']);

          $filtro=array_filter($this->dados($filtro));


          if(!empty($ordem)){

              $this->db->order_by('PRODUTO.nome',$ordem);

          }

        $this->db->where('PRODUTO.exclusao is null');


        if(!empty($filtro)){

            $filtro = array_filter($filtro);

            $this->db->like($filtro);

        }

        $this->banco->usuario("PRODUTO");

        return $this->db->get()->num_rows();

    }



}
